...
raw_data = """plaid beige bags contain 3 drab magenta bags.
...
def CanHold(bag):
    parent_bags = []
    for key in rules:
        for rule, value in rules[key].items():
            if rule == bag:
                parent_bags.append(key)
    return(parent_bags)

def HastoHold(input_bag):
    child_bags = []
    for bag in rules[input_bag[0]].items():
        if bag[0] != 'other bags':
            child_bags += [(bag[0], int(bag[1]) * input_bag[1])]
    return child_bags

data = raw_data.split("\n")
rules = [line.split('contain') for line in data]
rules = {rule[0][:-6]: {" ".join(item.split()[1:3]):item.split()[0] for item in rule[1][1:-1].split(',')} for rule in rules}

needs_tested = []
tested_bags = []
needs_tested += CanHold(mybag)
# tested_bags.append(mybag) #doesn't need to hold itself

new_bags = []

while needs_tested:
    BagToTest = needs_tested.pop()
    new_bags = CanHold(BagToTest)
    tested_bags.append(BagToTest)

    for bag in new_bags:
        if bag not in needs_tested and bag not in tested_bags:
            needs_tested.append(bag)
print('Answer 1: {}'.format(len(tested_bags)))
tested_bags = []
mybag = (mybag, 1)
needs_tested += HastoHold(mybag)

while needs_tested:
    BagToTest = needs_tested.pop()
    needs_tested += HastoHold(BagToTest)
    tested_bags.append(BagToTest)


totalbags = 0

for item in tested_bags:
    totalbags += item[1]

print('Answer 2: {}'.format(totalbags))