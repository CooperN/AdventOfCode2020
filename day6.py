...
for party in data:
    unique_party_answers = []
    same_letters = 0
    same_party_answers = []
    party_count = 0
    for person in party.split('\n'):
        if not party_count:
            same_party_answers = list(person)
        same_party_answers = [letter for letter in same_party_answers if letter in list(person)]
        for letter in person:
            if letter not in unique_party_answers:
                unique_party_answers.append(letter)
        party_count += 1
    sameanswers.append(same_party_answers)
    num_same_answers += len(same_party_answers)
    uniqueanswers.append(unique_party_answers)
    num_unique_answers += len(unique_party_answers)
print('Question 1: ' + str(num_unique_answers))
print('Question 2: ' + str(num_same_answers))
